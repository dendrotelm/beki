...
const i18n = {
  en: {
    ...
    controlsMapMouse: [['WASD','Move'],['Mouse','Aim'],['LMB (Hold & Drag)','Power + Curve Shot'],['LMB (Tap)','Flick (Lob)'],['C','Pass (Ground)'],['Space','Jump / Wallride'],['Shift','Sprint']],
    controlsMapKbd: [['WASD','Move'],['F (Hold)','Straight Power Shot'],['Q (Hold) + A/D','Curve Shot'],['E','Flick (Lob)'],['C','Pass (Ground)'],['Space','Jump / Wallride'],['Shift','Sprint']],
    ...
    leaderboard: "🏆 Leaderboard", elo: "Your Rating:", loading: "Loading...",
  ...
  pl: {
    ...
    controlsMapMouse: [['WASD','Ruch (Kontrola w locie)'],['Myszka','Celowanie'],['LPM (Trzymaj i Przeciągnij)','Siła + Podkręcenie'],['LPM (Klik)','Krótka podcinka'],['C','Szybkie podanie'],['Spacja','Skok / Wallride'],['Shift','Sprint']],
    controlsMapKbd: [['WASD','Ruch (Kontrola w locie)'],['F (Klik)','Krótka podcinka'],['F (Trzymaj)','Prosty Strzał'],['Q (Trzymaj) + A/D','Podkręcony Strzał'],['Spacja','Skok / Wallride'],['Shift','Sprint']],
    ...
    leaderboard: "🏆 Ranking Globalny", elo: "Twoja Ranga:", loading: "Wczytywanie...",
...
function Menu({onStart, lang, setLang, theme, setTheme, onInteract, musicPlaying, setMusicPlaying}){
  ...
  useEffect(() => {
    ...
    socket.on('playerData', (data) => { if(data.elo) setPlayerElo(data.elo); });
    ...
    socket.on('gameStarted', (data) => { 
      ...
      onStart({ mode: isWaiting ? 'online-host' : 'online-client', role, roomId: roomId || joinCode, controls, difficulty: diff, playerColor, scoreLimit: limit, timeLimit: tLimit, is2v2: isWaiting ? is2v2 : remote2v2 }); 
    });
    socket.on('errorMsg', (msg) => { alert(msg); setIsWaiting(false); });
    
    return () => { 
      if(audioRef.current) audioRef.current.pause();
      if(socket) { socket.off('roomCreated'); socket.off('joinedRoom'); socket.off('lobbyUpdate'); socket.off('gameStarted'); socket.off('errorMsg'); socket.off('playerData'); socket.off('leaderboardData'); }
    };
  }, [isWaiting, joinCode, onStart, roomId, role, controls, diff, playerColor, scoreLimit, timeLimit, is2v2]);
  ...
  const handleCreateOnline = () => { if (!socket || !socket.connected) return alert(t.offlineErr); socket.emit('createRoom', {limit: scoreLimit, timeLimit, is2v2}); };
          ...
          <div style={{maxHeight: '300px', overflowY: 'auto', margin: '20px 0', padding: '10px', background: st.inputBg, borderRadius: 8}}>
            {leaderboardData.length === 0 ? <p>{t.loading}</p> : leaderboardData.map((player, idx) => (
                <div key={idx} style={{display:'flex', justifyContent:'space-between', padding:'8px', borderBottom: st.separator.borderTop}}>
                  ...
                  <span style={{color: '#00ffaa', fontWeight:'bold'}}>{player.elo} ELO</span>
                </div>
            ))}
          </div>
  ...
  if (isWaiting) return (
    <div style={st.wrapper}>
      <div style={{...st.card, textAlign:'center'}}>
        <h2>{t.waitOpponent} <span style={{color:isDark?'#00ffaa':'#9b1c9b'}}>{lobbyStatus.joined}/{lobbyStatus.max}</span></h2>
        <p style={{color: st.label.color}}>{t.giveCode}</p>
        <h1 style={{fontSize:56, letterSpacing:6, color:isDark?'#00ffaa':'#cc2200', textShadow: isDark ? '0 0 20px rgba(0,255,170,0.5)' : 'none', margin:'20px 0'}}>{roomId}</h1>
        <button onClick={()=>{setIsWaiting(false); socket.disconnect(); socket=null;}} style={{...st.btn('#ff4444', true), width:'auto', padding:'10px 30px'}}>{t.cancel}</button>
      </div>
    </div>
  );

  return(
    <div style={st.wrapper} onClick={onInteract}>
      <div style={{position:'absolute', top:0, left:0, right:0, bottom:0, backgroundImage: 'url(/abandonedpitch.jpg)', backgroundSize: 'cover', backgroundPosition: 'center', opacity: isDark ? 0.15 : 0.3, mixBlendMode: isDark ? 'overlay' : 'normal', pointerEvents:'none'}} />

      <div style={{position:'absolute', top: 20, right: 20, display:'flex', gap: 8, zIndex: 10}}>
        <button onClick={()=>{setMusicPlaying(!musicPlaying); onInteract();}} style={{padding:'6px 12px', cursor:'pointer', background:musicPlaying?'#ff4488':st.inputBg, color:'#fff', border:st.card.border, borderRadius:8, fontWeight:'bold'}}>
      ...
      <div style={st.card}>
        ...
        {!showJoin ? (
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
            {[ {id:'bot',label:t.playBot,color:isDark?'#00ffaa':'#1e6b36',onClick:()=>onStart({mode:'bot',difficulty:diff,controls,playerColor,scoreLimit,timeLimit,is2v2,role:'p1'})}, 
               {id:'multi',label:t.playLocal,color:isDark?'#3b82f6':'#1753c2',onClick:()=>onStart({mode:'multi',difficulty:diff,controls,playerColor,scoreLimit,timeLimit,is2v2,role:'p1'})}, 
...
function Game({mode, difficulty, roomId, role, controls, playerColor, scoreLimit, timeLimit, is2v2, onBack, lang}){
  ...
  const prevRTPressed = useRef(false);

  const touchInputs = useRef({ mx: 0, mz: 0, aimX: 0, aimZ: 1, chargeStraight: false, chargeCurve: false, jump: false, dash: false, flick: false, pass: false, spinCharge: 0 });
  const dragStartX = useRef(0); 
  const rightStickBase = useRef({ x: 0, y: 0 });

  // FUNKCJE DOTYKOWE - Wyciągnięte prawidłowo POZA useEffect
  ...
  const handleRightTouchMove = (e) => {
      ...
      touchInputs.current.spinCharge = dx * 0.1; 
  };
  const handleRightTouchEnd = () => { touchInputs.current.chargeCurve = false; touchInputs.current.spinCharge = 0; };
  ...
  useEffect(()=>{
    ...
    const trajPtsArr=[]; for(let i=0;i<60;i++) trajPtsArr.push(new THREE.Vector3());
    ...
    const mkAgent=(id, team)=>({id, team, pos:new THREE.Vector3(),vel:new THREE.Vector3(),onGround:false,chargeT:0,chargingStraight:false,chargingCurve:false,dir:new THREE.Vector3(0,0,1),dashCD:0,dashT:0,dashDirX:0,dashDirZ:0, kickCD:0, lobCD:0, spinCharge:0, pass: false});
    ...
    let remoteInputs = {
      p2: { mx: 0, mz: 0, aimX: 0, aimZ: -1, jump: false, flick: false, chargeStraight: false, pass: false, dash: false, spinReq: 0 },
      p3: { mx: 0, mz: 0, aimX: 0, aimZ: 1, jump: false, flick: false, chargeStraight: false, pass: false, dash: false, spinReq: 0 },
      p4: { mx: 0, mz: 0, aimX: 0, aimZ: -1, jump: false, flick: false, chargeStraight: false, pass: false, dash: false, spinReq: 0 }
    };
    ...
    const keyUp = e => { if(isTyping.current) return; inputState.keys[e.code] = false; };
    ...
    const onMouseUp = (e) => {
      if(controls!=='mouse' || isMobile) return;
      if (e.button === 0) inputState.lmbDown = false;
    };
    ...
    const softContact=(ball,agent,dt)=>{
      ...
      const adjDist = Math.sqrt(dx*dx+dy*dy+dz*dz) || 1;
      const minDist = BR+PR;
    ...
    const doKick=(agent,power,type,spinReq)=>{
      ...
      if (type === 'flick') {
        ...
        ball.vel.x = fX * 18 * power; ball.vel.z = fZ * 18 * power; 
        ball.vel.y = 1.0; ball.spin = 0; 
      } else {
        fX = agent.dir.x; fZ = agent.dir.z;
        const len=Math.sqrt(fX*fX+fZ*fZ)||1;
        fX /= len; fZ /= len;
      ...
      if(type !== 'flick' && type !== 'pass' && power > 1.2) triggerShake(0.15, power * 0.15); 
    ...
    const moveAgent=(agent,mx,mz,aimX,aimZ,jump,flick,chargeStraight,passPressed,dashPressed,spinReq,spd,halfSign,dt)=>{
      ...
      if (chargeStraight) {
        ...
        if (agent.chargeT < 0.25) {
            ...
            doKick(agent, 1.0 + agent.chargeT*1.5, 'straight', spinReq); 
        }
        agent.chargeT = 0; agent.chargingStraight = false; agent.spinCharge = 0;
      }

      if(flick && !agent.chargingStraight) {
         if(doKick(agent, 1.0, 'flick', 0)) agent.lobCD = 0.3;
      }
      
      if(passPressed && !agent.chargingStraight) {
         if(doKick(agent, 0.8, 'pass', 0)) agent.lobCD = 0.3;
      }
    ...
    const runBot = (agent, isTeamA, dt) => {
        ...
        if (agent.botDelay <= 0) {
            ...
            if (dd > 0.2) {
                if (Math.abs(agent.pos.x) > hw - PR - 0.5 && Math.sign(dx) === Math.sign(agent.pos.x)) {
                    agent.botInp.mx = -Math.sign(agent.pos.x) * 0.5; 
                    agent.botInp.mz = Math.sign(dz) || 1; 
                } else {
                    agent.botInp.mx = dx/dd; agent.botInp.mz = dz/dd; 
                }
            } else { agent.botInp.mx=0; agent.botInp.mz=0; }
    ...
    const simulateBallStep = (bVel, bPos, bSpin, sdt) => {
        ...
        if (Math.abs(bSpin) > 0.05) {
            ...
            if (speed2D > 0.5) {
                ...
                bVel.x += pX * bSpin * sdt * 26.0; 
                bVel.z += pZ * bSpin * sdt * 26.0;
                bVel.x *= 1.0 - (0.12 * sdt); 
                bVel.z *= 1.0 - (0.12 * sdt);
            }
        }
    ...
    const update=(dt)=>{
      ...
      if (!isMatchOver && goalTimer <= 0 && timeLimit > 0) {
          ...
          if (timerSeconds <= 0) {
              ...
              if (score.a > score.b) {
                  label = mode.startsWith('online') ? (myTeam === 'a' ? t.winnerYou : t.winnerOpp) : (is2v2 ? t.winnerTeamA : t.winnerP1 || "Wygrana!");
              } else if (score.b > score.a) {
                  label = mode.startsWith('online') ? (myTeam === 'b' ? t.winnerYou : t.winnerOpp) : (is2v2 ? t.winnerTeamB : t.winnerP2 || "Przegrana!");
              } else {
                  label = t.draw; 
              }
      ...
      let chargeStraightL = false, flickL = false, passL = false;
      let aimX=0, aimZ=1;
      ...
      if (gp) {
          ...
          if (Math.abs(gp.axes[0]) > 0.15 || Math.abs(gp.axes[1]) > 0.15) {
              mxLocal = gp.axes[0]; mzLocal = gp.axes[1];
          }
          
          let rightStickL = Math.sqrt(gp.axes[2]**2 + gp.axes[3]**2);
          if (rightStickL > 0.15) {
              aimX = gp.axes[2] / rightStickL; aimZ = gp.axes[3] / rightStickL;
          } else { aimX = myP.dir.x; aimZ = myP.dir.z; }

          if (gp.buttons[0]?.pressed) jumpL = true; 
          if (gp.buttons[1]?.pressed) dashL = true; 
          if (gp.buttons[2]?.pressed || gp.buttons[3]?.pressed) passL = true; 
          if (gp.buttons[5]?.pressed || gp.buttons[4]?.pressed) flickL = true; 
          
          const rtPressed = gp.buttons[7]?.pressed || gp.buttons[6]?.pressed;
          if (rtPressed) {
              chargeStraightL = true;
              myP.spinCharge = clamp(myP.spinCharge + gp.axes[2] * dt * 15, -6.0, 6.0);
          } else {
              myP.spinCharge = 0;
          }
      }
      ...
      if (isMobile) {
        ...
        chargeStraightL = ti.chargeStraight; 
        jumpL = ti.jump; dashL = ti.dash;
        passL = ti.pass; touchInputs.current.pass = false; 
        
        if (ti.chargeCurve) myP.spinCharge += ti.spinCharge * 15.0 * dt; 
        if(ti.flick) { chargeStraightL = true; setTimeout(() => { touchInputs.current.chargeStraight = false; }, 50); ti.flick = false; }
        if (!ti.chargeCurve) ti.spinCharge = 0; 
      } else if (!gpActive) {
        mxLocal=(inputState.keys['KeyD']?1:0)-(inputState.keys['KeyA']?1:0); 
        mzLocal=(inputState.keys['KeyS']?1:0)-(inputState.keys['KeyW']?1:0);
        jumpL = !!inputState.keys['Space']; 
        dashL = !!inputState.keys['ShiftLeft'];
        passL = !!inputState.keys['KeyC'];
        ...
        if (controls === 'mouse') {
          ...
          if (chargeStraightL) {
             ...
             myP.spinCharge = clamp(deltaX * 10.0, -6.0, 6.0); 
          } else {
             myP.spinCharge = 0;
          }
        } else {
          chargeStraightL = !!inputState.keys['KeyF'];
          if(inputState.keys['KeyE']) flickL = true;
          ...
          if (chargeStraightL) {
            if (inputState.keys['KeyA']) myP.spinCharge += dt * 15; 
            if (inputState.keys['KeyD']) myP.spinCharge -= dt * 15; 
          } else {
             myP.spinCharge = 0;
          }
        }
      ...
      myP.spinCharge = clamp(myP.spinCharge, -6.0, 6.0);
      ...
      if (mode === 'online-client') {
        if (serverState) {
          ...
          if (serverState.goalTimer > 0 && goalTimer <= 0) { goalTimer = serverState.goalTimer; msgText = serverState.msgText; }
          serverState = null;
        }
        
        moveAgent(myP, mxLocal, mzLocal, aimX, aimZ, jumpL, flickL, chargeStraightL, passL, dashL, spinReq, 9, myP.team==='A'?1:-1, dt);
        agents.forEach(ag => { if(ag !== myP) moveAgent(ag, 0,0, ag.dir.x, ag.dir.z, false, false, false, false, false, 0, 9, ag.team==='A'?1:-1, dt); });
        
        physicsTick(dt);
      } 
      else {
        moveAgent(p1, mxLocal, mzLocal, aimX, aimZ, jumpL, flickL, chargeStraightL, passL, dashL, spinReq, 9, +1, dt);
        
        if (mode === 'online-host') {
          const runOpp = (ag) => {
             const inp = remoteInputs[ag.id];
             if(inp) moveAgent(ag, inp.mx, inp.mz, inp.aimX, inp.aimZ, inp.jump, inp.flick, inp.chargeStraight, inp.pass, inp.dash, inp.spinReq, 9, ag.team==='A'?1:-1, dt);
             else moveAgent(ag, 0,0, ag.dir.x, ag.dir.z, false, false, false, false, false, 0, 9, ag.team==='A'?1:-1, dt);
          };
          runOpp(p2);
          if (is2v2) { runOpp(p3); runOpp(p4); }
        } else if(mode === 'multi') {
          const mx2=(inputState.keys['ArrowRight']?1:0)-(inputState.keys['ArrowLeft']?1:0);
          const mz2=(inputState.keys['ArrowDown']?1:0)-(inputState.keys['ArrowUp']?1:0);
          ...
          if (inputState.keys['Numpad0']) {
             if (inputState.keys['ArrowLeft']) spinReq2 += dt * 15;
             if (inputState.keys['ArrowRight']) spinReq2 -= dt * 15;
          }
          p2.spinCharge = clamp(p2.spinCharge + spinReq2, -6.0, 6.0);
          if (!inputState.keys['Numpad0']) p2.spinCharge = 0;

          moveAgent(p2, mx2, mz2, aimX2, aimZ2, !!inputState.keys['ShiftRight'], !!inputState.keys['Numpad2'], !!inputState.keys['Numpad0'], !!inputState.keys['Numpad3'], !!inputState.keys['Enter'], p2.spinCharge, 9, -1, dt);
        ...
        if(ball.pos.z <= -hd - 0.15) { 
          if(inGX && inGY && !isMatchOver && goalTimer <= 0){ score.a++; msgText='GOL!'; goalTimer=2.5; pendingReset='A'; triggerShake(0.5, 0.8); if(socket&&mode==='online-host') socket.emit('gameEvent',{roomId, type:'shake', power:0.8}); } 
        }
        if(ball.pos.z >= hd + 0.15) { 
          if(inGX && inGY && !isMatchOver && goalTimer <= 0){ score.b++; msgText='GOL!'; goalTimer=2.5; pendingReset='B'; triggerShake(0.5, 0.8); if(socket&&mode==='online-host') socket.emit('gameEvent',{roomId, type:'shake', power:0.8}); } 
        }
      }

      if (mode.startsWith('online') && socket && netTick >= NETWORK_RATE) {
        if (mode === 'online-client') {
          socket.emit('clientInput', { roomId, role, input: { mx: mxLocal, mz: mzLocal, aimX, aimZ, jump: jumpL, flick: flickL, chargeStraight: chargeStraightL, pass: passL, dash: dashL, spinReq } });
        } else {
          let stateObj = {
              ball: { pos: ball.pos, vel: ball.vel, spin: ball.spin },
              p1: { pos: p1.pos, vel: p1.vel, chargeT: p1.chargeT, dashT: p1.dashT, dashCD: p1.dashCD, dir: p1.dir, onGround: p1.onGround },
      ...
      if(myP.chargeT>0.05 && inRange){
        const power=1.0+myP.chargeT*1.5;
        let ax = myP.dir.x; 
        let az = myP.dir.z;
        const al=Math.sqrt(ax*ax+az*az)||1;
        
        let sVel = new THREE.Vector3((ax/al)*17*power, myP.onGround ? 5 + power * 2.5 : 8 + power * 3, (az/al)*17*power);
        let sPos = ball.pos.clone();
        let sSpin = myP.spinCharge * power;
        const newPts=[];
        const simDt = 0.016; 
    ...
    return()=>{
      ...
      if(socket) { socket.off('opponentInput'); socket.off('gameState'); socket.off('triggerEvent'); socket.off('chatMessage'); socket.off('matchEnded'); }
  ...
  },[mode, difficulty, roomId, role, controls, playerColor, scoreLimit, timeLimit, is2v2, lang, t]);
        ...
        {isMobile && !gameOver && (
          <>
            <div onTouchStart={handleLeftTouchMove} onTouchMove={handleLeftTouchMove} onTouchEnd={handleLeftTouchEnd}
                 style={{position:'absolute',bottom:40,left:40,width:130,height:130,borderRadius:'50%',background:'rgba(255,255,255,0.1)',border:'2px solid rgba(255,255,255,0.3)', touchAction:'none'}}>
                 <span style={{position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,-50%)',color:'#fff',opacity:0.5,fontSize:10, pointerEvents:'none'}}>MOVE</span>
            </div>
            
            <div onTouchStart={handleRightTouchStart} onTouchMove={handleRightTouchMove} onTouchEnd={handleRightTouchEnd}
                 style={{position:'absolute',bottom:40,right:40,width:130,height:130,borderRadius:'50%',background:'rgba(255,255,255,0.1)',border:'2px solid rgba(255,255,255,0.3)', touchAction:'none'}}>
                 <span style={{position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,-50%)',color:'#fff',opacity:0.5,fontSize:10, pointerEvents:'none'}}>AIM/CURVE</span>
            </div>

            <button onTouchStart={()=> { touchInputs.current.pass = true; setTimeout(()=>touchInputs.current.pass=false, 50); }} style={{position:'absolute',bottom:110,right:190,width:50,height:50,borderRadius:'50%',background:'rgba(200,200,200,0.3)',color:'#fff',border:'none', touchAction:'none'}}>PASS</button>
            <button onTouchStart={()=> { touchInputs.current.flick = true; setTimeout(()=>touchInputs.current.flick=false, 50); }} style={{position:'absolute',bottom:180,right:120,width:50,height:50,borderRadius:'50%',background:'rgba(255,160,0,0.3)',color:'#fff',border:'none', touchAction:'none'}}>FLICK</button>
            <button onTouchStart={()=>touchInputs.current.jump=true} onTouchEnd={()=>touchInputs.current.jump=false} style={{position:'absolute',bottom:180,right:50,width:50,height:50,borderRadius:'50%',background:'rgba(0,160,255,0.3)',color:'#fff',border:'none', touchAction:'none'}}>JUMP</button>
            <button onTouchStart={()=>touchInputs.current.dash=true} onTouchEnd={()=>touchInputs.current.dash=false} style={{position:'absolute',bottom:40,right:190,width:50,height:50,borderRadius:'50%',background:'rgba(0,255,100,0.3)',color:'#fff',border:'none', touchAction:'none'}}>DASH</button>
          </>
        )}
